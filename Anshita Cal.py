...
root.title("Anshita Scientific Calculator") 
...
def special_keys(event):
    if event.keysym == "Return":
        ...
        txtDisplay.delete(0, END)
        txtDisplay.insert(0, current[:-1] if len(current) > 1 else "0")

    elif event.keysym == "Escape":
        added_value.All_Clear_Entry()

# Bind keyboard
root.bind("<Key>", key_input)
root.bind("<Return>", special_keys)
root.bind("<BackSpace>", special_keys)
root.bind("<Escape>", special_keys)
